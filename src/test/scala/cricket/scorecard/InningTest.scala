...
import cricket.scorecard.InningTest.{bowlersRepository, teamRepository}
import cricket.scorecard.models.{Bowler, Out, Over, Player, Runs, Stats, Wicket}
...
class InningTest extends AnyWordSpec with Matchers {
  "nextBall" should {
    "update score" in {
      ...
      val matchScoreBoard = Inning(mockScoreCard, 1, teamRepository, bowlersRepository)
    ...
    "change player on strike when its a wicket and more players are remaining" in {
      ...
      val matchScoreBoard = Inning(scoreCard, 1, teamRepository, bowlersRepository)
    ...
    "update out-players list when a player gets out on non last over balls" in {
      ...
      val inning = Inning(scoreCard, 1, teamRepository, bowlersRepository)
    ...
    "update out-players list when a player gets out on last ball of over" in {
      ...
      val inning = Inning(mockScoreCard, 1, teamRepository, bowlersRepository)
    ...
    "mark end of inning when last wicket falls" in {
      ...
      val inning = Inning(scoreCard = mockCard, 1, team, bowlersRepository)
    ...
    "mark end of innings when all overs are played" in {
      ...
      val inning = Inning(scoreCard, 1, team, bowlersRepository)
...
private object InningTest {
  ...
  val bowlersRepository: BowlersRepository = BowlersRepository.createFrom(List(Bowler("A"), Bowler("B"), Bowler("C"), Bowler("D")))
}
